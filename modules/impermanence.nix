...
}:
{
  options = {
    ...
    impermanence.extraPersistDirectories = lib.mkOption {
  ...
  config =
    let
      ...
      impermanenceDirs = defaultImpermanenceDirs ++ config.impermanence.extraPersistDirectories;
    ...
    lib.mkIf config.impermanence.enable {
      ...
      systemd.tmpfiles.rules = lib.lists.flatten (
        [
          "L+ /persist/etc/machine-id -    -    -    -    /etc/machine-id"
